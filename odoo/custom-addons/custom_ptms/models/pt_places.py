...
class PTPlace(models.Model):
    ...
    # SKILLS FIELD
    skill_ids = fields.Many2many('pt.skill', string="Required Skills (At least One skill)")
    ...
    def action_view_skills(self):
        ...
        if not skill_ids:
            return {
                ...
                'params': {
                    'title': 'No Skills',
                    'message': 'No skills are required for this place.',
                    'type': 'info',
                    'sticky': False,
                }
            }
        return {
            'name': f'Skills for {self.name}',
            'type': 'ir.actions.act_window',
            'res_model': 'pt.skill',
            'view_mode': 'tree',
            'view_id': self.env.ref('custom_ptms.pt_skill_popup_tree_view').id,
            'target': 'new',  # Opens in a popup
            'domain': [('id', 'in', skill_ids.ids)],
