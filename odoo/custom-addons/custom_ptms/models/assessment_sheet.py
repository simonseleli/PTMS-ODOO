...
class AssessmentSheet(models.Model):
    ...
    def write(self, vals):
        """Override write to trigger recomputation of marks in pt.place.application."""
        result = super().write(vals)
        # Check if any mark-related fields were updated
        mark_fields = [
            'training_officer', 'logbook_content', 'logbook_neatness', 'logbook_relevance',
            'logbook_organization', 'academic_supervisor', 'report_intro', 'report_safety',
            'report_job_desc', 'report_recruitment', 'task_diagram', 'task_problem',
        ...
        if any(field in vals for field in mark_fields):
            _logger.info(f"Assessment sheet {self.id} updated, triggering mark recomputation for application {self.application_id.id}")
            self.application_id._compute_assessment_marks()
        return result
    ...
    def _adjust_marks(self):
        ...
        for field, max_mark in max_marks.items():
            ...
            if value < 0 or value > max_mark:
                setattr(self, field, 0)  # Reset to default value
                return {
                    'warning': {
                        'title': 'Invalid Input',
                        'message': f"{field.replace('_', ' ').title()} must be between 0 and {max_mark}. The value has been reset to 0.",
                    }
                }
        return None