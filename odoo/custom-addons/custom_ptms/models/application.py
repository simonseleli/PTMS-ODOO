...
class PtApplication(models.Model):
    ...
    #     'student.student',
    #     string="Student",
    #     default=lambda self: self._get_student()
    # )
    ...
    #                           ('accepted', 'Accepted'), ('rejected', 'Rejected')], string="Status", readonly=True,
    #                          default='draft', track_visibility='onchange', tracking=True)
    # vacancy_id = fields.Many2one(
    #     'pt.company.vacancy', string="Company", domain="[('remaining_chances','>',0)]", readonly=True,
    ...
    # partner_id_self = fields.Many2one('res.partner', string="Company", domain="[('is_company','=',True)]",
    ...
    #     'academic.year', string="Academic Year", domain="[('current','=',True)]",
    ...
    #     string="Confirmation Letter", readonly=True,
    #     states={'draft': [('readonly', False)], 'submitted': [('readonly', False)]}, attachment=True)
    ...
    # assignment_user_id = fields.Many2one('res.users', related='assignment_id.teacher_id.employee_id.user_id',
    #                                      store=True, string="Supervisor")
    #
    ...
    #
    # ########################Grading Information######################
    # # Update marks fields to be computed
    # officer_marks = fields.Float(
    #     "Officer Marks", compute='_compute_assessment_marks', store=True
    # )
    # logbook_marks = fields.Float(
    #     "Log Book Marks", compute='_compute_assessment_marks', store=True
    # )
    # supervisor_marks = fields.Float(
    #     "Supervisor Marks", compute='_compute_assessment_marks', store=True
    # )
    # report_marks = fields.Float(
    #     "Final Report Marks", compute='_compute_assessment_marks', store=True
    # )
    # total_marks = fields.Float(
    #     "Total Marks", compute='_compute_assessment_marks', store=True
    # )
    #
    ...
    #     'pt.application.assessment.sheet', 'application_id', string="Assessment Sheets"
    # )
    # ##########################Final Report############################
    ...
    #                                                                       'standard_id', '=',
    #                                                                       rec.student_id.standard_id.id),
    #                                                                   ('division_id', '=',
    #                                                                    rec.student_id.standard_id.division_id.id)])
    ...
    #                                                                         'given_chances': 1, 'assigned_chances': 1})
    #                 rec.vacancy_id = vacancy.id
    #         rec.state = 'accepted'
    #
    ...
    #                                ('state', 'in', ['draft', 'submitted', 'accepted'])])
    ...
    #         }
    #
    # @api.depends('assessment_sheet_ids.total', 'assessment_sheet_ids.training_officer',
    #              'assessment_sheet_ids.logbook_content', 'assessment_sheet_ids.logbook_neatness',
    #              'assessment_sheet_ids.logbook_relevance', 'assessment_sheet_ids.logbook_organization',
    #              'assessment_sheet_ids.academic_supervisor', 'assessment_sheet_ids.report_intro',
    #              'assessment_sheet_ids.report_safety', 'assessment_sheet_ids.report_job_desc',
    #              'assessment_sheet_ids.report_recruitment', 'assessment_sheet_ids.task_diagram',
    #              'assessment_sheet_ids.task_problem', 'assessment_sheet_ids.task_solution',
    #              'assessment_sheet_ids.task_comparison', 'assessment_sheet_ids.task_technical',
    #              'assessment_sheet_ids.task_conclusion', 'assessment_sheet_ids.task_references',
    #              'assessment_sheet_ids.task_formatting')
    # def _compute_assessment_marks(self):
    #     for rec in self:
    #         # Default to 0 if no assessment sheet
    #         rec.officer_marks = 0.0
    #         rec.logbook_marks = 0.0
    #         rec.supervisor_marks = 0.0
    #         rec.report_marks = 0.0
    #         rec.total_marks = 0.0
    #
    #         # Use the latest assessment sheet (or the one marked as final)
    #         assessment = rec.assessment_sheet_ids.sorted(
    #             lambda a: (a.is_final, a.create_date), reverse=True
    #         )[:1]
    #         if assessment:
    #             rec.officer_marks = assessment.training_officer
    #             rec.logbook_marks = sum([
    #                 assessment.logbook_content, assessment.logbook_neatness,
    #                 assessment.logbook_relevance, assessment.logbook_organization
    #             ])
    #             rec.supervisor_marks = assessment.academic_supervisor
    #             rec.report_marks = sum([
    #                 assessment.report_intro, assessment.report_safety,
    #                 assessment.report_job_desc, assessment.report_recruitment,
    #                 assessment.task_diagram, assessment.task_problem,
    #                 assessment.task_solution, assessment.task_comparison,
    #                 assessment.task_technical, assessment.task_conclusion,
    #                 assessment.task_references, assessment.task_formatting
    #             ])
    #             rec.total_marks = assessment.total
    #
    # def action_open_assessment_sheet(self):
    #     self.ensure_one()
    #     # Find or create the assessment sheet
    ...
    #         'target': 'current',  # Changed from 'new' to 'current' for better UX
    #     }